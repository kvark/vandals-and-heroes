...
fn create_collider(content_pack: &ContentPack, desc: &ColliderDesc) -> rapier3d::geometry::Collider {
    let builder = match &desc.shape {
        ShapeDesc::Box { size: (hx, hy, hz) } => rapier3d::geometry::ColliderBuilder::cuboid(*hx, *hy, *hz),
...
impl ObjectTemplate {
    pub fn instantiate(
        ...
        let model_instance = self.model
            .as_ref()
            .map(|m| ModelInstance {
                model: m.clone(),
                transform
            });
        let body = self.desc.physics
            .as_ref()
            .map(|p| {
                let colliders = p.colliders.iter()
